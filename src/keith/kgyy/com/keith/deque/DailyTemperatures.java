...
public class DailyTemperatures {
     ...
     * 2. 利用deque.peek() 预览当前即将出队的元素，不出对，用来对应的温度跟当前温度对比
    ...
    public int[] dailyTemperature20251112(int[] temperatures) {
        int length = temperatures.length;
        Deque<Integer> deque = new LinkedList<>();
        int[] ans = new int[length];
        for(int i = 0;i<length;i++) {
    ...
    public int[] dailyTemperature2025(int[] temperatures) {
        int length = temperatures.length;
        Deque<Integer> deque = new LinkedList<>();
        int[] ans = new int[length];
        for (int i = 0;i<length;i++) {
            int tem = temperatures[i];;
            while(!deque.isEmpty() && tem > temperatures[deque.peek()]) {
