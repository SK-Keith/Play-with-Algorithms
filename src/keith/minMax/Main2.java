...
//        });
        ...
        Map<Integer, List<GoodTrend>> mapGs = new HashMap<>();
        for (GoodTrend gt1 : gts1) {
            if (map.containsKey(gt1.getLastVal())) {
                mapGs.get(gt1.getLastVal()).add(gt1);
            } else {
                List<GoodTrend> array = new ArrayList<>();
                array.add(gt1);
                mapGs.put(gt1.getLastVal(), array);
            }
        }

        List<GoodTrend> results = new ArrayList<>();
        int key = 0;
        Map<Integer, String> trendMap = new HashMap<>();
//        while (true) {
//            results = mapGs.get(key);
//            if (results.size() > 0) {
//                trend.append(key + "->");
//                for (GoodTrend gt : gts) {
//                    trend.append(gt.getWhichOne()).append("->");
//                    trendMap.put(gt.getMaxVal(), trend.toString());
//                }
//            }
//        }


        Map<Integer, List<GoodTrend>> allNum = new HashMap<>();
        long count = gts.stream().filter(gt -> gt.getLastVal() == 0).count();
        for (GoodTrend gt : gts) {
            if (!allNum.containsKey(gt.getWhichOne())) {
                List<GoodTrend> goodTrends = allNum.get(gt.getLastVal());
                goodTrends.add(gt);
                allNum.put(gt.getWhichOne(), goodTrends);
            } else {
                List<GoodTrend> goodTrends = new ArrayList<>();
                goodTrends.add(gt);
                allNum.put(gt.getWhichOne(), goodTrends);
            }
        }
        ...
        allNum.forEach((A, B) -> {
            trend.append(A).append("->");
            B.forEach(b -> {
                trend.append(b.getWhichOne());
            });
        });
...
//        while (true) {
//            List<GoodTrend> maxList = gts1.stream().filter(m1 -> m1.getMaxVal() == maxVal).collect(Collectors.toList());
//            maxList.forEach(m2 -> {
//
//            });
//            for (GoodTrend gt1 : gts1) {
//                if (gt1.getMaxVal() == maxVal) {
//                    maxVal = gt1.getLastVal();
//                    trend =  trend + "->" + gt1.getWhichOne();
//                    val = gt1.getLastVal();
//                    break;
//                }
//            }
