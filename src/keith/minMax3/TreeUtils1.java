...
public class TreeUtils1 {
     ...
     */
    public void queryAll(Node changeNode, int V, int M, int[] v, int[] m, int[] w, List<String> prints) {
        List<Node> sonList = changeNode.getSonList();
//        String space = generateSpace(depth);    //根据深度depth,返回(depth*3)长度的空格
        if (sonList == null || sonList.isEmpty()) {
            String[] split = changeNode.getIndexs().substring(1).split(",");
            int weightSum = 0, volumnSum = 0, valueSum = 0;
            List<String> indexs = new ArrayList<>();
            StringBuilder strs = new StringBuilder();
            for (String s : split) {
                int i = Integer.parseInt(s);
                if (volumnSum + v[i] > V || weightSum + m[i] > M) {
                    break;
                }
                volumnSum += v[i];
                weightSum += m[i];
...
//            System.out.print("\t，展开分析总体积为：" + volumnSum + "，总重量为：" + weightSum + "，总价值为：" + valueSum);
            if (!prints.contains(strs.toString())) {
                prints.add(strs.toString());
            }
...
//            }
        ...
        for (int i = 0; i < sonList.size(); i++) {
            ...
            queryAll(sonList.get(i), V, M, v, m, w, prints);
        }
